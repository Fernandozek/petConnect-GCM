...
export default function Register() {
  ...
  async function createProfile(user, dados:any) {
    const token:any = await api.getToken({body: {
      username: data.email,
      password: data.senha,
    }})
    
    let res = await api.createProfile({
      token: token.access,
      body: {
  ...
  async function validate() {
    ...
    let res = await api.createUser({
      body: { username: data.email, password: data.senha, email: data.email },
    });
    ...
    if (res.statusCode) {
      ...
      await createProfile(res, dataCopy);
    }
  }
  return (
      ...
      <div className="w-full h-full px-32 flex items-center flex-col overflow-hidden">
        <div className="w-full h-[85%] p-[4%] relative">
          ...
          <img
            ...
            className={`absolute bottom-0 right-0 ${
              activeForm && selectedOption === "option2" ? "w-[25%]" : "w-[30%]"
            }`}
          />
        </div>
        <div className="w-full flex items-end pt-1 justify-end border-t-2 border-secondary">
          ...
          <Button
            ...
            color="primary"
            handle={!activeForm ? () => {setActiveForm(true); setData({ ...emptyData });} : validate}
          />
        </div>
      </div>
    </PageTemplate>
